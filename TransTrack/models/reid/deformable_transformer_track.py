...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300, reid_shared=False,
        ...
        self.reid_shared = reid_shared
        
        if not reid_shared:
            self.decoder_reid = DeformableTransformerDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None,): 
        ...
        hs_reid = hs
        if not self.reid_shared: 
            hs_reid, inter_references_reid = self.decoder_reid(tgt, reference_points, memory,
                                                      spatial_shapes, valid_ratios, query_embed, mask_flatten)

        if self.two_stage and self.training:
            return hs, hs_reid, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact, memory
        
        return hs, hs_reid, init_reference_out, inter_references_out, None, None, memory
